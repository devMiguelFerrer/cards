...
export class GenericRepository<T extends { id: number }>
  ...
  async add(data: T): Promise<T> {
    return await getConnection('mysql-typeorm').getRepository(this.entity).save(data);
  }

  async update(newData: T): Promise<T> {
    let oldData = await getConnection('mysql-typeorm')
      .getRepository(this.entity)
      .findOne(newData.id);
  ...
  async remove(id: string): Promise<T> {
    return await getConnection('mysql-typeorm').getRepository(this.entity).delete(id) as any;
  }
